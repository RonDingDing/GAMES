...
namespace CGL
{
    ...
    void Application::init()
    {
        ...
        // Create two ropes
        int num = 10;
        double damping_factor = 0.0005;
        ropeEuler = new Rope(Vector2D(0, 200), Vector2D(-400, 200), num, config.mass,
                             config.ks, {0}, damping_factor);
        ropeVerlet = new Rope(Vector2D(0, 200), Vector2D(-400, 200), num, config.mass,
                              config.ks, {0}, damping_factor);
    }
